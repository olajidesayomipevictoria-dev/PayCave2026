...
// is no clean wallet_transactions reference to reconcile against.

db.manualCreditUser = function (telegramId, amount, adminId, reason) {
    return new Promise((resolve, reject) => {
        const reference = `MC-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
            ...
            db.run(
                ...
                function (err) {
                    ...
                    db.run(
                        `INSERT INTO wallet_transactions
                         (telegram_id, reference, amount, type, status)
                         VALUES (?, ?, ?, 'MANUAL_CREDIT', 'SUCCESS')`,
                        [telegramId, reference, amount],
                        function (txErr) {
                            if (txErr) {
                                db.run("ROLLBACK");
                                return reject(txErr);
                            ...
                            db.run(
                                ...
                                [
                                    ...
                                    `Credited ₦${amount} to user ${telegramId}. Reason: ${reason || "N/A"}`
                                ],
                                (logErr) => {
                                    if (logErr) {
                                        db.run("ROLLBACK");
                                    ...
                                    db.run("COMMIT", (commitErr) => {
                                        if (commitErr) return reject(commitErr);
                                        resolve({ ok: true, reference, amount, telegramId });
                                    });
                                }
                            );
                        }
